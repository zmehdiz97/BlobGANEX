...
from utils import get_tensor_value, opt_var, devide_layout, assemble
...
class Config:
    ...
    num_imgs=200 # used if real_images=False and load_inversion=False

    invert_only=False # to just run the inverion optim (no cf)

    #to load already existing inverions (stores in 'shared_files/')
    load_inversion=True
...
class BlobEX():
    ...
    def cf_optim(self, metadata, images, im_tar):
        ...
        λ_prox = 0.01 
        λs = {'xs':6, 'ys':6, 'sizes':1, 'covs':1, 'features':20, 'spatial_style':20}
        target_blobs = [14,18,23,29,30,35,38]
        ...
        for target_attribute in target_attributes:
            ...
            devided_layout_metadata = devide_layout(layout_metadata_orig, target_blobs)
            devided_layout_metadat_opt = opt_var(devided_layout_metadata, target_params=target_features)
            # defining params and lr
            params = []
            for key, val in devided_layout_metadat_opt.items():
                if key in target_features:
            ...
            for step in pbar:
                layout_metadat_opt = assemble(devided_layout_metadat_opt, layout_metadata_orig, target_blobs)
